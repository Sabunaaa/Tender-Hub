...
from tender_scraper.engagements import EngagementError, add_engagement, list_engagements, update_engagement


def _insert_tender(conn, app_id: int, number: str, buyer: str = "City Hall"):
    conn.execute(
        """
        INSERT INTO tenders (
        ...
        ) VALUES (?, 'k', ?, 'ქსელური მოწყობილობები', 'Tender announced', 'NAT', ?,
...
def test_update_engaged_and_managers(tmp_repo):
    ...
    with_domain = update_engagement(row["id"], {"domain": "storage"}, db)
    assert with_domain["domain"] == "Storage"

    try:
        update_engagement(row["id"], {"domain": "PCs"}, db)
        raise AssertionError("expected error")
    except EngagementError as exc:
